...
maxT = sys.argv[4]
namea = "test_tank"
add_aquarium = {"auth_token":auth_token,
                "aquarium": {"name":namea,
                             "ideal_temperature":ideal_T,
                             "ideal_temp_low":minT,
                             "ideal_temp_high":maxT}}
aquarium_data = json.dumps(add_aquarium)
url_add_aqua = url +"aquariums"
aquarium_done = requests.post(url_add_aqua, aquarium_data)
name_aquarium = "mine"

class aquarium(object):

    #send temperature and ph to readings to aquarimeter web
    def send_readings(self, ph, temp):
        reading_data = {"auth_token":auth_token,
                        "reading": {"ph":ph,
                                    "temperature":temp}}
        send_read = json.dumps(reading_data)
        url_send = url + name_aquarium +/readings"
        sent_data = requests.post(url_send, send_read)

    #recieves update on name and/or temperature
    def recieve_update(self):
        update_request = {"auth_token":auth_token,
                          "aquarium": {"name":"foobar"}}
